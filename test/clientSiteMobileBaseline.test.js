...
test('injectClientSiteMobileBaseline skips when already present', () => {
  const h = `<!DOCTYPE html><html><head><style id="data-cs-mobile-baseline">x{}</style></head><body></body></html>`;
...
test('injectClientSiteMobileBaseline inserts style before closing head', () => {
  ...
  assert.ok(out.includes('data-cs-mobile-baseline'));
  assert.ok(out.includes('max-width:100%'));
  const iStyle = out.indexOf('data-cs-mobile-baseline');
