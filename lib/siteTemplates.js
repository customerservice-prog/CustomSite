...
(function(){ var pid='__CS_PROJECT_UUID__'; var f=document.getElementById('cs-contact-form');
  if(f) f.addEventListener('submit', function(e){ e.preventDefault(); var fd=new FormData(f), body={};
    fd.forEach(function(v,k){ body[k]=v; }); body.current_url = typeof location !== 'undefined' ? location.href : ''; var st=document.getElementById('cs-form-status');
    fetch('/api/forms/'+pid+'/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),credentials:'omit'})
      .then(function(r){return r.json().catch(function(){return{};}).then(function(j){return{ok:r.ok,j:j};});})
...
.pro { max-width: 40rem; }
.grid { display: grid; gap: 1.25rem; grid-template-columns: repeat(auto-fit, minmax(min(12rem, 100%), 1fr)); }
...
<body>
  ...
  <section id="contact" class="section ec-contact"><div class="wrap"><h2>Questions?</h2>
    <form id="cs-contact-form" style="max-width:24rem;margin:1rem auto 0;text-align:left">
      <p><label>Name<br /><input type="text" name="name" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #cbd5e1" /></label></p>
      <p><label>Email<br /><input type="email" name="email" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #cbd5e1" /></label></p>
      <p><label>Message<br /><textarea name="message" rows="4" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #cbd5e1"></textarea></label></p>
      <button type="submit" class="btn" style="width:100%">Send message</button>
      <p id="cs-form-status" aria-live="polite" style="margin-top:.75rem;font-size:.875rem;color:#475569"></p>
    </form></div></section>
...
.plans { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(14rem, 100%), 1fr)); gap: 1rem; }
...
.ec-contact { background: #f8fafc; }
.ec-contact h2 { text-align: center; }
`,
...
<body>
  ...
  <section id="contact" class="contact"><div class="wrap"><h2>Start a project</h2>
    <form id="cs-contact-form" style="max-width:22rem;margin:1rem auto 0;text-align:left">
      <p><label>Name<br /><input type="text" name="name" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #d6d3d1" /></label></p>
      <p><label>Email<br /><input type="email" name="email" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #d6d3d1" /></label></p>
      <p><label>Message<br /><textarea name="message" rows="4" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #d6d3d1"></textarea></label></p>
      <button type="submit" class="btn-submit" style="width:100%;padding:0.65rem;background:#0c0a09;color:#fafaf9;border:none;border-radius:0.35rem;font-weight:600;cursor:pointer">Send</button>
      <p id="cs-form-status" aria-live="polite" style="margin-top:.75rem;font-size:.875rem;color:#57534e;text-align:center"></p>
    </form></div></section>
...
(function(){ var pid='__CS_PROJECT_UUID__'; var f=document.getElementById('cs-contact-form');
  if(f) f.addEventListener('submit', function(e){ e.preventDefault(); var fd=new FormData(f), body={};
    ...
    fetch('/api/forms/'+pid+'/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),credentials:'omit'})
      .then(function(r){return r.json().catch(function(){return{};}).then(function(j){return{ok:r.ok,j:j};});})
      .then(function(x){if(st) st.textContent=x.ok&&x.j.ok?'Thanks — message received.':(x.j.error||'Error.');})
      .catch(function(){ if(st) st.textContent='Network error.'; });
  }); })();
  </script>
  <script src="app.js"></script>
...
<body>
  ...
  <section id="contact" class="section"><div class="wrap c">
    <h2>Reserve or message</h2>
    <form id="cs-contact-form" style="max-width:22rem;margin:1rem auto 0;text-align:left">
      <p><label style="display:block;font-size:.85rem;margin-bottom:.25rem">Name</label><input type="text" name="name" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #e7e5e4" /></p>
      <p><label style="display:block;font-size:.85rem;margin-bottom:.25rem">Email</label><input type="email" name="email" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #e7e5e4" /></p>
      <p><label style="display:block;font-size:.85rem;margin-bottom:.25rem">Message</label><textarea name="message" rows="3" required style="width:100%;padding:0.55rem;border-radius:0.35rem;border:1px solid #e7e5e4"></textarea></p>
      <button type="submit" class="btn" style="width:100%;border:none;cursor:pointer">Send message</button>
      <p id="cs-form-status" aria-live="polite" style="margin-top:.75rem;font-size:.875rem;color:#57534e"></p>
    </form>
    <p style="margin-top:1.25rem"><a class="btn" href="tel:+15550000000">Or call to reserve</a></p></div></section>
  <script>
(function(){ var pid='__CS_PROJECT_UUID__'; var f=document.getElementById('cs-contact-form');
  if(f) f.addEventListener('submit', function(e){ e.preventDefault(); var fd=new FormData(f), body={};
    fd.forEach(function(v,k){ body[k]=v; }); body.current_url = typeof location !== 'undefined' ? location.href : ''; var st=document.getElementById('cs-form-status');
    fetch('/api/forms/'+pid+'/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),credentials:'omit'})
      .then(function(r){return r.json().catch(function(){return{};}).then(function(j){return{ok:r.ok,j:j};});})
      .then(function(x){if(st) st.textContent=x.ok&&x.j.ok?'Thanks — we saved your note.':(x.j.error||'Error.');})
      .catch(function(){ if(st) st.textContent='Network error.'; });
  }); })();
  </script>
  <script src="app.js"></script>
